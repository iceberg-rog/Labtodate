...
import { bulkArchiveTickets, bulkUnarchiveTickets, bulkDeleteTickets } from '@/lib/support/actions';

type Row = TicketRowProps;

/**
 * Wraps the ticket list with selection + bulk actions. Server page passes a
 * flat array of TicketRow props; this component handles all client-side
 * state (selected set, action invocation, refresh).
 *
 *   - In the active queue: Archive selected
 *   - In the archived tab: Restore selected, Delete selected (capability-gated)
 */
export function TicketBulkList({
  rows,
  view,
  canDelete,
}: {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="sticky top-0 z-30 flex items-center gap-2 flex-wrap rounded-2xl border-2 border-primary/40 bg-primary/[0.06] backdrop-blur px-4 py-2.5 shadow-sm">
          ...
          {view === 'active' && (
            <button
              type="button"
              disabled={pending}
              onClick={() => run(bulkArchiveTickets)}
          ...
          {view === 'archived' && (
            <>
              <button
                type="button"
                disabled={pending}
                onClick={() => run(bulkUnarchiveTickets)}
                className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full border border-border bg-card text-xs font-bold hover:bg-foreground/5 disabled:opacity-50"
              >
              ...
              {canDelete && (
                <button
                  ...
                  disabled={pending}
                  onClick={() => {
                    if (!confirm(`Permanently delete ${selected.size} ticket${selected.size === 1 ? '' : 's'}? This cannot be undone.`)) return;
                    run(bulkDeleteTickets);
                  }}
      ...
      {msg && (
        <p className="text-xs text-emerald-700 font-semibold px-1">{msg}</p>
      )}
